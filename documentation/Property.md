# Property & Contact Information

## Lifesaver Labs Public Benefit Corporation

This document consolidates information about Lifesaver Labs PBC's property, including domain names and telephone numbers reserved for the Neighbor 911 / Naybor SOS™ project.

---

## Telephone Numbers

### United States Toll-Free Number
**+1-844-2-911-119** (UHH-2-911-119)

- **Vanity Spelling:** UHH-2-911-119
- **Purpose:** Primary toll-free emergency support line for Neighbor 911 services
- **Coverage:** United States toll-free calling
- **Format:** +1 (844) 2-911-119

### United States Local Number
**442-2-911-123** (HI-22-911-123)

- **Vanity Spelling:** HI-22-911-123
- **Purpose:** Local access number for Neighbor 911 services
- **Advantages:**
  - Local number avoids toll-free charges
  - Cost-effective in an era where most users have prepaid phone service or unlimited calling plans
  - Reduces duplicative billing infrastructure overhead
- **Coverage:** Area code 442 (Southern California region)
- **Format:** (442) 2-911-123

---

## Reserved Domain Portfolio

We've secured emergency number domains to support international interoperability and future expansion. These domains remain available for development as the project grows:

| Domain | .org | .com | .us | .app | .ai |
|--------|------|------|-----|------|----|
| neighbor112 | ✅ | ✅ | ✅ | — | - |
| neighbor119 | ✅ | ✅ | ✅ | — | - |
| neighbor911 | ✅ | ✅ | ✅ | ✅ | ✅ |
| neighbor000 | ✅ | - | ✅ | - | - |
| neighbor100 | ✅ | - | ✅ | - | - |
| neighbor767 | ✅ | ✅ | - | - | - |
| neighborsos | Taken | Taken | ✅ | ✅ | - |
| nghbr911 | - | - | ✅ | — | - |
| nghbrsos| ✅ | ✅ | ✅ | — | - |
| neighbour911 | ✅ | ✅ | ✅ | — | - |

The current thinking is for neighbor911.us (more UNited States) and neighbor911.org (more global and international) to be the principal internet domains and web frontends, with the rest mirroring or forwarding back to those two primaries.

---

## Trademark Protection

For information about trademark strategy and intellectual property, see [Intellectual Property.md](Intellectual Property.md).

Key trademarks include:
- **Neighbor 911™** (core trademark)
- **Naybor SOS™** (evolving global brand identity)
- Multilingual variants and emergency number variants

---

## Seed Funding & Financial Support

### Founder's Commitment
The founder of Lifesaver Labs PBC is contributing **$18,000 from personal funds** as seed funding to launch the Neighbor 911 / Naybor SOS™ project.

### We Welcome Additional Support
We are actively seeking contributions from others to help fuel and fund this project. **Startup funding is critical** to:
- Spin up pilot calmunities (communities) effectively
- Test and refine our best designs in real-world scenarios
- Scale out proven solutions rapidly across neighborhoods, cities, and regions
- Build the infrastructure and support systems needed for a life-saving emergency response network

Emergency response technology benefits from strong network effects—the more communities that participate, the more lives we can save. Your financial support helps us reach that critical mass faster.

**Interested in supporting this work?**
Contact us at team@lifesaverlabs.org to discuss funding opportunities, partnerships, or other forms of support.

---

## Contact Information

### General Inquiries
- **Email:** team@lifesaverlabs.org
- **Website:** [lifesaverlabs.org](https://lifesaverlabs.org)

### Neighbor 911 Support
- **Primary Toll-Free:** +1-844-2-911-119
- **Local Access:** 442-2-911-123

### Safeword™ Project
- **Email:** team@safeword.app

---

**Last Updated:** 2025-11-24
